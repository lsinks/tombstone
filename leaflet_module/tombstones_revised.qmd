...
Now split this into two dataframes
...
resetting all the photo folders

This moves all the files into the main folder.

**Need to delete files in the folders sfirs**t
...
```{r}
matched_first_round <- inner_join(photo_df, tombstones_merged)

index <- 1
for (index in seq(1:nrow(matched_first_round))) {
  file.rename(from = here(blog_folder, photo_folder, unmatched_folder, matched_first_round$photo_names[index]),
                         to = here(blog_folder, photo_folder, match1, matched_first_round$photo_names[index] ))
}

```

Get an error even though it looks like it moved some files.
...
```{r}
multiples <- tombstones_merged %>% group_by(full_name_MI) %>% count(sort = TRUE) %>% filter(n > 1)
```
...
df_temp <- tombstones_merged %>%
  filter(full_name_MI == multiples$full_name_MI[1]) 
df_temp
...
for (index in seq(1:nrow(multiples))) {
  df_temp <- tombstones_merged %>%
    filter(full_name_MI == multiples$full_name_MI[index])
  df_temp
  these <-
...
FILE STRUCTYRE BELOW HERE UNTESTED

```{r}
photo_names2 = list.files(here(blog_folder, photo_folder, match1))
...
I think this produces one more than it should in photo names. 75 instead of 74 via code below vs looking at \# files in the folders.
...
unmatched_middle <- tombstones_matched %>%
  ...
  filter(is.na(photo_names2) == TRUE)
...
```{r}
photo_names3 = list.files(here(blog_folder, problem_folder))
match_round2 = as.data.frame(photo_names3)
```
...
tombstones_matched2 <- fuzzy_right_join(match_round2, unmatched_middle, 
                             by = c("photo_names3" = "full_name_MI_no_space"),
                             match_fun = str_detect)
```
...
for (index in seq(1:nrow(multiples2))) {
  ...
  df_temp
  these <-
    as.list(here(blog_folder, problem_folder, df_temp$photo_names3))
  photo_panel <-
    image_append(do.call("c", lapply(these, image_read)))
  image_write(
    photo_panel,
  ...
  for (index2 in seq(1:nrow(df_temp))) {
    file.rename(
      from = here(blog_folder, problem_folder, df_temp$photo_names[index2]),
...
I need to move the non-panel pictures I matched.

```{r}
 index2 <- 1
  for (index2 in seq(1:nrow(tombstones_matched2))) {
    if (is.na(tombstones_matched2$photo_names3[index2] ) == FALSE) {
    file.rename(
      from = here(blog_folder, problem_folder, tombstones_matched2$photo_names3[index2]),
      to = here(blog_folder, photo_folder, tombstones_matched2$photo_names3[index2])
    )
    }
  }
```
