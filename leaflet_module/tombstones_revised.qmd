...
Create the dataframe of files to match.
...
```{r}
duplicate_names <- tombstones_middle %>% group_by(full_name_MI) %>% count(sort = TRUE) %>% filter(n > 1)
```
...
```{r}
multiple_photos <- tombstones_merged %>% group_by(full_name_MI) %>% count(sort = TRUE) %>% filter(n > 1)
```
...
df_temp <- tombstones_merged %>%
  filter(full_name_MI == multiple_photos$full_name_MI[1]) 
df_temp
...
for (index in seq(1:nrow(multiple_photos))) {
  df_temp <- tombstones_merged %>%
    filter(full_name_MI == multiple_photos$full_name_MI[index])
  df_temp
  these <-
