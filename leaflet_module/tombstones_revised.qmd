  ...
  function(df_to_match, name_to_match, match_folder) {
    ...
    if (nrow(multiple_photos) > 0) {
      ...
      for (index in seq(1:nrow(multiple_photos))) {
        name1 <- multiple_photos[index, 1]
        name2 <- "Follis A O"
        name3 <- multiple_photos[[name_to_match]][index]
...
MAKE THIS A FUNCTION CALLED UPDATE PHOTO NAMES
...
```{r}
tester2 <- Matching_Photos(unmatched_middle, "full_name_MI_no_space", match2)
```

```{r}
photo_names_r2 = list.files(here(blog_folder, photo_folder, match2))
photo_df_MI_final2 = as.data.frame(photo_names_r2)
tombstones_matched <- fuzzy_right_join(photo_df_MI_final2, tombstones_matched, 
...
```{r}
tester3 <- Matching_Photos(unmatched_full, "full_name", match3)
```

That actually doesn't seem too bad.

```{r}
photo_names_r3 = list.files(here(blog_folder, photo_folder, match3))
photo_df_MI_final3 = as.data.frame(photo_names_r3)
tombstones_matched <- fuzzy_right_join(photo_df_MI_final3, tombstones_matched, 
                             by = c("photo_names_r3" = "full_name"),
                             match_fun = str_detect)
```

Sinks Arlie and kin seem to be the main matching problem

```{r}
tombstones_matched_final <- tombstones_matched %>%
  mutate(photo_list = ifelse(
    is.na(photo_names_r1) == TRUE, 
    ifelse(is.na(photo_names_r2) == TRUE, photo_names_r3, photo_names_r2)
...
tombstones_matched_final2 <- tombstones_matched_final %>%
  select(photo_list, photo_names_r1, photo_names_r2, photo_names_r3)
```
