...
The MarkerClusters and the popup with the picture need to be linked by group. There is a pop-up generated in the addCircleMarkers with the text info. When `addPopupImages()` is called, it just appends the photos to that text. I couldn't find a way to directly add text to the `addPopupImages()` call.
