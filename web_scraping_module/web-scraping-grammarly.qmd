...
Web scraping is frequently against the terms of service of many websites. High-volume, high-speed web scraping is also frequently a problem as it can overload the website you are scraping.
...
There is a simple example on the [webscraping using rvest](https://rvest.tidyverse.org/articles/rvest.html). This vignette illustrates basic HMTL and CSS elements and shows you how to extract them. The rvest webpage recommends using the [SelectorGadget](https://rvest.tidyverse.org/articles/selectorgadget.html) to identify the elements you want to extract. I didn't use that, but the linked page does have a good explanation.

You can view the source data for your webpage (usually by right clicking in the browser and clicking view source) and just look for the elements you want. This is what I did.

This is what the webpage looks like. It doesn't look particularly complicated. There are 74 rows in the table.
...
There are 2260 lines of code involved in making that webpage. I looked for the first name in the table PHELPS in the source code. It shows up at line 451. This is the entire line:
...
A table should be tagged as a table with

<table>

. Each row should be tagged with
...
HTML is hierachical, and the relationship between the elements is described in the language of families- parent elements, children elements, etc. I want to take the children elements of the table.

I need to understand how the row is structured, so I can select the right element. A row contains 5 cells. The first row is a header row.
...
1.  Select the elements we want out of the HTML using html_nodes. The xpath code specifies that we want a tags that are children of the table. Store it in link_nodes.
2.  An alternate method is to use the more standard HTML/CSS selectors, but again, we want the a elements that are children of the table. Commented out here, but it produces identical results as 1.
...
5.  Now we loop through the text dataframe and add on the matching URL that we extract from link_nodes using html_attr("href").
...
Now I am subsetting the dataframe using subset_index for the rows and taking all columns.
...
You can see it produces the same results.

## Dataframes aren't Tibbles!

R keeps the indexing of the raw_data, so the row numbers are by 3s. However, indexing is as usual, by position. So rev_war\$name_unclean\[3\] returns the name associated with the row number 7. This is annoying and makes it difficult to pull out test cases. So, this should be fixed. This issue will pop up when you use data frames but not when you use tibbles. The tidyverse doesn't approve of row names or numbers, so they aren't used. If you look at a tibble in RStudio viewer, you will get row numbers and the tibble will have them correctly numbered. Tibbles and data frames are usually entirely interchangeable, but sometimes you do run into the differences.
...
```{r}
rev_war_tibble <- as.tibble(raw_data)
...
I admit I used trial and error to figure this out. First note- using `str_split()` from stringr is not the way to go. I got hyperfocused on stringr functions since I was using the package so heavily. It creates a matrix of results in your data frame and not new columns of data. Use `separate()` from tidyr instead.
...
My plan was to match on the first variation, then for those that failed to match, match on the second variation, and so on. I was mildly hopeful that the first variation was the variation that my father used in his Excel table. So, I did a quick check using an inner join. My father had mentioned that there wouldn't be many matches- maybe 5 or 6, because he hadn't found most of the graves yet.
...
Let's see who didn't match because there were no DOB or DOD.
...
What we see is that many names are reused in the family, and so the names from the SAR records match people who were born well after the end of the Revolutionary War. I could try to clean up by date of birth or death, though that information isn't available for all matches. The SAR records don't have the middle names in the title; it's probably easier just to ask. Jr/Sr is not consistently encoded in my father's spreadsheet (which is why I didn't use that when generating match_1). At this point, subject matter expertise is needed to solve the problem.
