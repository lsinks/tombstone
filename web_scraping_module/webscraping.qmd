...
#  tombstones_raw <-
#    read.xlsx(here("leaflet_module", data_file),
#      sheet = 1
#    )
...
# Conceptual Overview of Webscraping

You want to find the HTML elements that correspond to the data you wish to obtain. There are a few ways to do this. One is to right click on the webpage in your browser and chose "inspect" to find out what elements you want to use (choose the elements tab). Many items may have the same elements, so you probably need to construct sets of elements to get to the right items.
...
matched_records <- rev_war_test %>% inner_join(data_file, by = c("match_1" = "full_name"))

dob_filter <- matched_records %>% select(name_unclean, Surname, First.Name, Middle.Name, DOB_date, DOD_date ) %>% filter(is.na(DOB_date) == FALSE) %>% filter(year(DOB_date) < 1784)
```
...
```{r}
all_matches <- dob_filter %>% full_join(dod_filter)
```

Final results

```{r}
all_matches %>% select(name_unclean) %>% gt()
```
